...
class AddAnalysisState(StatesGroup):
    waiting_for_id = State()
    

async def add_analisis_chat(message: types.Message, session: Session, state: FSMContext) -> None:
    await state.clear()
    if await user_get_id_pyrogram() == message.from_user.id:
        ...
        await state.set_state(AddAnalysisState.waiting_for_id)
        await state.update_data(user_chats=chats)

        return message.answer(
                    'Для того чтобы добавить чат для анализа\n'
...
async def add_analisys_chat_by_id(message: types.Message, session: Session , state: FSMContext):
    # Отмена
    ...
    # result: ScalarResult = await session.execute(select(AnalysisChat).join(User).where(User.telegram_user_id==message.from_user.id,
    #                             AnalysisChat.chat_id==chat['id']))
    result = session.query(AnalysisChat.id).join(User).filter(User.telegram_user_id==message.from_user.id,
                                AnalysisChat.chat_id==chat['id']).count() > 0
        
    if not result:
        user: User = session.query(User).filter(User.telegram_user_id == message.from_user.id).first()

        new_mailing_chat = AnalysisChat(
            user_id=user.id,
            chat_id=chat['id'],
            chat_name=chat['name']
        )
